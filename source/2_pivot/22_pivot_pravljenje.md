...
Kliknite na *Insert* (1), *PivotTable* (2) i odaberite opciju *From Table/Range* (3).

![Korak 2](images/pivot2_sr.png)

### Korak 3: Odaberite gdje želite da se nalazi vaša pivot tablica
Možete odabrati novi radni list (*New Worksheet*) ili lokaciju na istom radnom listu (*Existing Worksheet*) (4) (u tom slučaju kliknite na ćeliju u kojoj će biti gornji lijevi kut vaše pivot tablice) (5). Potvrdite klikom na *Ok*. (6)
...
### Korak 5: Dodajte redove (*Rows*) i vrijednosti (*Values*)
...
```{infonote}
Ako se dogodi da vam se zatvori prozor s desne strane koji omogućuje podešavanje prikaza pivot tablice, možete ga ponovno otvoriti tako da kliknete na bilo koju ćeliju pivot tablice i odaberete Show field list.
```
### Korak 7: Dodajte filtre (opcionalno)
Dodavanje filtara omogućit će vam da iz velike količine podataka brzo izdvojite i prikažete samo one vrijednosti koje su vam potrebne, bez izmjene početne tablice i dodatnih izračuna.

```{infonote}
Iako je pivot tablica povezana s originalnom tablicom, izmjene u njoj se ne ažuriraju automatski. Nakon svake izmjene desnim klikom na pivot tablicu odaberite opciju Refresh kako bi se svi rezultati osvježili.
```
## Pivot grafikon
