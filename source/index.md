---
status: exclude
alias: cocreate-health-habits_cro
short_description: Analiza podataka o zdravim navikama
long_description: >
    <p>Ovaj kurs uvodi učenike u analizu podataka pomoću pivot tablica i pivot grafikona. Kroz praktičan rad na računalu, učenici uče kako organizirati, grupirati, filtrirati i vizualno prikazati podatke, kao i kako iz njih izvesti zaključke. Tijekom kursa učenici će uočiti razlike između običnih i pivot tablica i grafikona te postupno razvijati sigurnost u analizi podataka kroz suradnički rad...</p>
    <p>
    <a rel="license" href="https://creativecommons.org/licenses/by/4.0/deed.sr_LATN">
    <img alt="Creative Commons License" style="border-width:0" src="https://i.creativecommons.org/l/by/4.0/88x31.png"></a>
    <br>This course was published by Petlja Foundation under the <a rel="license" href="https://creativecommons.org/licenses/by/4.0/deed.sr_LATN">Creative Commons Attribution 4.0 International License (CC BY 4.0)</a>.
    </p>
will_learn:
    - Što je pivot tablica i po čemu se razlikuje od obične tablice
    - Kako se kreira i podešava pivot tablica
    - Kako se koriste redovi, stupci, vrijednosti i filtri
    - Kako se kreira pivot grafikon i vizualiziraju rezultati
    - Kako se tumače podaci i formuliraju zaključci
    - Kako se suradnički analiziraju podaci i predstavljaju rezultati


needed: 
    - Računalo s instaliranim programom Microsoft Excel ili pristupom Google Sheets
useful:
    - Python 3.12 documentation: https://docs.python.org/3.12/
---

# Analiza i vizualizacija podataka

Započinjete uvodni kurs iz analize podataka u kojem ćete naučiti koristiti pivot tablice i pivot grafikone za organiziranje, analizu i vizualni prikaz podataka. Kroz rad na računalu i suradnju s drugim učenicima istraživat ćete kako se podaci mogu pretvoriti u jasne zaključke i smislene informacije.

Course author: Petlja Foundation

This course is published by Petlja Foundation under the
[Creative Commons Attribution 4.0 International License (CC BY 4.0)](https://creativecommons.org/licenses/by/4.0/deed.sr_LATN).


```{toctree}
:maxdepth: 2
:caption: Sadržaj

1_uvod/index
2_pivot/index
3_zadaci/index
4_prilozi/index
```